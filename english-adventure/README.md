...
- `stroke-data.js`：46 個平假名的筆順編號與正規化位置資料。只保存筆順事實與自製標記，不包含或重新發布第三方 PDF 圖像。
...
故事成功生成後才更新 `lastStoryUsed` 和 `storyUseCount`。L0/L1 回覆會在瀏覽器本機檢查常見非簡單現在式、必用單字、句數與句長；不會為了檢查再呼叫一次 AI。不合格時提示重新生成，由使用者決定是否再次消耗 API。
...
- 日文故事 L0 選 3–4 個複習單字，L1 選 5–8 個；所有級別都要檢查必用單字及設定字數。
...
- 五十音書寫畫布使用 Klee One／教科書體淡色字形，並依 `stroke-data.js` 在每一筆起點附近顯示 `1、2、3…` 圓形小標記。畫布維持可直接描寫，不加入動畫、箭頭或自動判分。
